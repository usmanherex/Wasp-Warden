...
const Navbar = () => {
  ...
  const handleMarkAsRead = async (notificationId) => {
    try {
      const userId = JSON.parse(localStorage.getItem("user"))?.userId;
      await axios.put(
        `http://localhost:5000/api/notifications/${notificationId}/read/${userId}`
      );
  ...
  const handleDelete = async (notificationId) => {
    try {
      const userId = JSON.parse(localStorage.getItem("user"))?.userId;
      await axios.delete(
        `http://localhost:5000/api/notifications/${notificationId}/${userId}`
      );
      await fetchNotifications();
    } catch (error) {
  ...
  return (
    <nav className="bg-gradient-to-r from-green-700 to-green-600 shadow-lg sticky top-0 z-10">
