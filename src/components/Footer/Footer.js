    ...
    <footer className="text-gray-500 bg-white px-4 py-5 max-w-screen-xl mx-auto md:px-8">
        ...
        <div className="flex-1">
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="max-w-sm flex items-center border rounded-md p-1">
              ...
              />
              <button className="p-2.5 rounded-md text-white bg-yellow-600 outline-none shadow-md focus:shadow-none sm:px-5">
                Subscribe
              </button>
            </div>
          </form>
          ...
          {footerNavs.map((item, idx) => (
            <ul className="space-y-4" key={idx}>
              ...
              {item.items.map((el, idx) => (
                <li key={idx}>
                  <Link to={el.href} className="hover:underline hover:text-indigo-600">
                    {el.name}
                  </Link>
                </li>
              ))}
      ...
      <div className="mt-8 py-6 border-t items-center justify-between sm:flex">
        <div className="mt-4 sm:mt-0">
          &copy; {new Date().getFullYear()} Wasp Warden. All rights reserved.
        </div>
