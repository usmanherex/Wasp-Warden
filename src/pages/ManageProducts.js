...
const ManageProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      let url;
      if (userType === "Agri-business") {
        url = `http://localhost:5000/agribusiness-products/${userId}?userType=${userType}`;
      } else {
        url = `http://localhost:5000/farmer-products/${userId}`;
      }
      ...
      if (data.success) {
        setProducts(userType === "Agri-business" ? data.data : data.products);
  ...
  const handleEdit = (productId) => {
    if (userType === "Agri-business") {
      navigate(`/edit-product-agribusiness/${productId}`);
    } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      let url;
      if (userType === "Agri-business") {
        url = `http://localhost:5000/agribusiness-product/${selectedProduct.itemId}`;
      } else {
        url = `http://localhost:5000/farmer/products/${selectedProduct.id}`;
      }
      ...
      if (data.success) {
        ...
        setProducts(
          products.filter(
            (p) =>
              (userType === "Agri-business" ? p.itemId : p.id) !==
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => handleEdit(product.id || product.itemId)}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-md w-full p-6">
              ...
              <p className="text-gray-600 mb-4">
                Are you sure you want to delete "{selectedProduct.itemName || selectedProduct.title}"?
