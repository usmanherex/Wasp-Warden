...
const CartPage = () => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/cart/items/${userId}`
      );
  ...
  const removeItem = async (cartId, itemId) => {
    try {
      const response = await axios.delete(
        `http://localhost:5000/cart/delete/${cartId}/${itemId}`
      );
      if (response.data.success) {
        fetchCartItems();
  ...
  const handleCheckout = () => {
    ...
    const items = cartItems.map((item) => ({
      ...
      hasNegotiatedPrice: item.hasNegotiatedPrice,
    }));
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3">
          {cartItems.map((item) => (
            <div
              key={item.itemId}
              className="bg-white rounded-lg shadow-md mb-6 hover:shadow-lg transition-shadow duration-300"
            >
              <div className="p-6">
            ...
            <div className="p-6">
              ...
              <div className="mt-6 space-y-3">
                <button
                  ...
                  className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-lg transition-colors duration-300"
                >
                  Proceed to Checkout
                </button>
                <button
